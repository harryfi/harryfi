                ...
                foreach (var item in accountinDbPaging)
                {
                    if (item.DatabasePathErasoft != null && item.DatabasePathErasoft != "")
                    {
                        ...
                        try
                        {
                            ...
                            sSQL += "select 'Order' as tipe,count(*) jumlah from hangfire.job (nolock) where statename='Enqueued' and InvocationData like '%Order%' and InvocationData not like '%GetOrdersToUpdateMO%' and InvocationData not like '%ShopeeControllerJob%GetOrderByStatusCompleted%JD_GetOrderByStatusComplete%'" + System.Environment.NewLine;
                            sSQL += "union all" + System.Environment.NewLine;
                            ...
                            sSQL += "select 'Manage' as tipe,count(*) jumlah from hangfire.job (nolock) where statename='Enqueued' and (InvocationData like '%GetOrdersToUpdateMO%' or InvocationData like '%ShopeeControllerJob%GetOrderByStatusCompleted%JD_GetOrderByStatusComplete%')" + System.Environment.NewLine;
