...
namespace MasterOnline.Controllers
{
    public class JDIDControllerJob : Controller
    {
        ...
        [AutomaticRetry(Attempts = 3)]
        [Queue("2_get_token")]
        public async Task<string> GetTokenJDIDV2(JDIDAPIData dataAPI, bool bForceRefresh)
        {
            string ret = "";
            DateTime dateNow = DateTime.UtcNow.AddHours(7);
            bool TokenExpired = false;
            ...
            if (TokenExpired || bForceRefresh)
            {
                ...
                MasterOnline.API_LOG_MARKETPLACE currentLog = new API_LOG_MARKETPLACE
                {
                    ...
                    REQUEST_ATTRIBUTE_2 = dataAPI.accessToken,
                    REQUEST_ATTRIBUTE_3 = dataAPI.no_cust,
                    REQUEST_STATUS = "Pending",
                };

                //ganti
                string urll = "https://oauth.jd.id/oauth2/to_login?app_key=" + dataAPI.appKey + "&response_type=code&redirect_uri=https://masteronline.co.id/&state=20200428&scope=snsapi_base";
                ...
                {
                    string urll2 = "https://oauth.jd.id/oauth2/access_token?app_key=" + dataAPI.appKey + "&app_secret=" + dataAPI.appSecret + "&grant_type=authorization_code&code=" + responseFromServer.Split('=').Last();
                    ...
                    if (listOrderId.success)
                    {
                        ...
                        if (listDetails != null)
                        {
                            ...
                            if (!string.IsNullOrEmpty(idOrderCancel))
                            {
                                ...
                                var rowAffected = EDB.ExecuteSQL("MOConnectionString", System.Data.CommandType.Text, "UPDATE SOT01A SET STATUS='2', STATUS_TRANSAKSI = '11', STATUS_KIRIM='5' WHERE NO_REFERENSI IN (" + idOrderCancel + ") AND STATUS_TRANSAKSI <> '11' AND CUST = '" + cust + "'");
