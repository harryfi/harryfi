    ...
    public class JDIDControllerJob : Controller
    {
        ...
        public string JD_GetOrderByStatusPaid(JDIDAPIDataJob iden, StatusOrder stat, string CUST, string NAMA_CUST, int page, int jmlhNewOrder)
        {
            ...
            //while (daysFrom >= -3)
            {
                ...
                var dateFrom = (long)daysNow.AddDays(-1).AddHours(-3).ToUniversalTime().Subtract(new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)).TotalMilliseconds;
