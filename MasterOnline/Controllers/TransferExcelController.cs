        ...
        public ActionResult ListBarangtoExcel(string kd_gudang)
        {
            ...
            try
            {
                using (var package = new OfficeOpenXml.ExcelPackage())
                {
                    ...
                    string sSQL = "select brg, nama + nama2 as nama from stf02 where type = 3";
                    var dsBarang = EDB.GetDataSet("CString", "STF02", sSQL);
                    ...
                    if (dsBarang.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if(gudang != null)
                        {
                            worksheet.Cells[1, 2].Value = gudang.Kode_Gudang;
                            for (int i = 0; i < dsBarang.Tables[0].Rows.Count; i++)
                            {
                            ...
                            //ret.byteExcel = package.GetAsByteArray();
                            //ret.namaFile = username + "_saldoawal" + ".xlsx";
                        }
                        else
                        {
                    ...
                    if (file != null && file.ContentLength > 0)
                    {
                        ...
                        using (MemoryStream stream = new MemoryStream(data))
                        {
                            ...
                            //using (ExcelPackage excelPackage = new ExcelPackage(existingFile))
                            {
                                ...
                                //foreach (ExcelWorksheet worksheet in excelPackage.Workbook.Worksheets)
                                //{
                                string gd = worksheet.Cells[1, 2].Value == null ? "" : worksheet.Cells[1, 2].Value.ToString();
                                if (!string.IsNullOrEmpty(gd))
                                {
                                    var gudang = ErasoftDbContext.STF18.Where(m => m.Kode_Gudang == gd).FirstOrDefault();
                                    if (gudang != null)
                                    {
                                        //string namaMP = mp.Where(m => m.IdMarket.ToString() == customer.NAMA).SingleOrDefault().NamaMarket;
                                        ...
                                        var listTemp = ErasoftDbContext.STF02.Where(m => m.TYPE == "3").ToList();
                                        if (listTemp.Count > 0)
                                        {
                                            #region create induk
                                            ...
                                            stt01a.Nobuk = noStok;
                                            ErasoftDbContext.STT01A.Add(stt01a);
                                            #endregion
                                            //loop all rows
                                                    ...
                                                    if (current_brg != null)
                                                    {
                                                        if (worksheet.Cells[i, 3].Value != null)
                                                        {
                                                            if (Convert.ToInt32(worksheet.Cells[i, 3].Value) > 0)
                                                            {
                                                                ...
                                                                ErasoftDbContext.STT01B.Add(stt01b);
                                                            }
                                                        }
