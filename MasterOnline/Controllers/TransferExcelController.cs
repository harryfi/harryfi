...
//            }
        ...
        public async Task<ActionResult> UploadXcelSaldoAwal(string nobuk, string _gudang, int countAll, string percentDanprogress, string statusLoopSuccess)
        {
            //var file = Request.Files[0];
            //List<string> excelData = new List<string>();
            //var listCust = new List<string>();
                                ...
                                if (checkTemp.Count() > 0)
                                {
                                    ...
                                    for (int j = Convert.ToInt32(prog[1]); j < checkTemp.Count(); j++)
                                    {
                                        try
                                        {
                                            ...
                                            //}
                                            ret.progress = j + 1;
                                            ret.percent = ((j + 1) * 100) / ret.countAll;
                                            //var bagiProses = (ret.countAll - 1) * (Convert.ToDecimal(30) / Convert.ToDecimal(100));
                                            //bagiProses = Decimal.Round(bagiProses);
                        ...
                        using (ErasoftContext eraDB = new ErasoftContext(DataSourcePath, dbPathEra))
                        {
                            ...
                            try
                            {
                                ...
                                for (int i = Convert.ToInt32(prog[0]); i <= worksheet.Dimension.End.Row; i++)
                                {
                                    ...
                                    //add by nurul 10/2/2022, multi lokasi 
                                    string gudang = worksheet.Cells[i, 21].Value == null ? "0" : worksheet.Cells[i, 21].Value.ToString();
                                    //end add by nurul 10/2/2022, multi lokasi
                                    //string disc1 = worksheet.Cells[i, 22].Value == null ? "0" : worksheet.Cells[i, 22].Value.ToString(); DIHAPUS
                                    string ndisc1 = worksheet.Cells[i, 22].Value == null ? "0" : worksheet.Cells[i, 22].Value.ToString();
                                    ...
                                    string total = worksheet.Cells[i, 23].Value == null ? "0" : worksheet.Cells[i, 23].Value.ToString();
            ...
            try
            {
                using (var package = new OfficeOpenXml.ExcelPackage())
                {
                    ...
                    string sSQL = "SELECT A.NO_BUKTI AS NO_PESANAN, NO_REFERENSI, A.TGL, E.NAMAMARKET + '(' + D.PERSO + ')' MARKETPLACE, A.PEMESAN AS KODE_PEMBELI, NAMAPEMESAN AS PEMBELI, " +
                        ...
                        //change by nurul 17/6/2021, tambah resi dan tlp pembeli 
                        ...
                        "ALAMAT_KIRIM, A.TERM AS [TOP], ISNULL(TRACKING_SHIPMENT,'') AS RESI,  SHIPMENT AS KURIR, TGL_JTH_TEMPO AS TGL_JATUH_TEMPO, A.KET AS KETERANGAN, A.BRUTO, A.NILAI_DISC AS DISC, A.PPN, A.NILAI_PPN, A.ONGKOS_KIRIM, A.NETTO, " +
                        //end change by nurul 17/6/2021, tambah resi dan tlp pembeli 
                        ...
                        ", ISNULL(A.WAREHOUSE_ID,'') AS WAREHOUSE_ID " +
                    ...
                    if (lsPesanan.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < lsPesanan.Tables[0].Rows.Count; i++)
                        {
                            ...
                            //ADD BY NURUL 22/3/2022
                            worksheet.Cells[5 + i, 12].Value = lsPesanan.Tables[0].Rows[i]["WAREHOUSE_ID"];
                            //END ADD BY NURUL 22/3/2022
                            worksheet.Cells[5 + i, 13].Value = lsPesanan.Tables[0].Rows[i]["TOP"];
                            worksheet.Cells[5 + i, 14].Value = Convert.ToDateTime(lsPesanan.Tables[0].Rows[i]["TGL_JATUH_TEMPO"]).ToString("yyyy-MM-dd HH:mm:ss");
                            worksheet.Cells[5 + i, 15].Value = lsPesanan.Tables[0].Rows[i]["KETERANGAN"];
                            worksheet.Cells[5 + i, 16].Value = lsPesanan.Tables[0].Rows[i]["BRUTO"];
                            worksheet.Cells[5 + i, 17].Value = lsPesanan.Tables[0].Rows[i]["DISC"];
                            worksheet.Cells[5 + i, 18].Value = lsPesanan.Tables[0].Rows[i]["PPN"];
                            worksheet.Cells[5 + i, 19].Value = lsPesanan.Tables[0].Rows[i]["NILAI_PPN"];
                            worksheet.Cells[5 + i, 20].Value = lsPesanan.Tables[0].Rows[i]["ONGKOS_KIRIM"];
                            worksheet.Cells[5 + i, 21].Value = lsPesanan.Tables[0].Rows[i]["NETTO"];
                            var pesanan1 = "";
                            ...
                            worksheet.Cells[5 + i, 23].Value = lsPesanan.Tables[0].Rows[i]["KODE_BRG"];
                            worksheet.Cells[5 + i, 24].Value = lsPesanan.Tables[0].Rows[i]["NAMA_BARANG"];
                            worksheet.Cells[5 + i, 25].Value = lsPesanan.Tables[0].Rows[i]["QTY"];
                            worksheet.Cells[5 + i, 26].Value = lsPesanan.Tables[0].Rows[i]["HARGA_SATUAN"];
                            worksheet.Cells[5 + i, 27].Value = lsPesanan.Tables[0].Rows[i]["DISC1"];
                            worksheet.Cells[5 + i, 28].Value = lsPesanan.Tables[0].Rows[i]["NDISC1"];
                            worksheet.Cells[5 + i, 29].Value = lsPesanan.Tables[0].Rows[i]["DISC2"];
                            worksheet.Cells[5 + i, 30].Value = lsPesanan.Tables[0].Rows[i]["NDISC2"];
                            worksheet.Cells[5 + i, 31].Value = lsPesanan.Tables[0].Rows[i]["TOTAL"];
                        }
        ...
        public ActionResult ExampleTemplatePesanantoExcel()
        {
            ...
            try
            {
                ...
                using (var package = new OfficeOpenXml.ExcelPackage())
                {
                    ...
                    table0.Columns[20].Name = "GUDANG";
                    //table0.Columns[20].Name = "DISC1";
                    table0.Columns[21].Name = "NDISC1";
                    //table0.Columns[22].Name = "DISC2";
                    //table0.Columns[23].Name = "NDISC2";
                    table0.Columns[22].Name = "TOTAL";

                    using (var range = worksheet.Cells[4, 1, 4, 23])
                    {
                        range.Style.Border.Top.Style = ExcelBorderStyle.Thin;
                        range.Style.Border.Left.Style = ExcelBorderStyle.Thin;
                        range.Style.Border.Right.Style = ExcelBorderStyle.Thin;
