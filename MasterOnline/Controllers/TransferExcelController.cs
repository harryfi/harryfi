                ...
                using (MemoryStream stream = new MemoryStream(data))
                {
                    using (ExcelPackage excelPackage = new ExcelPackage(stream))
                    {
                        ...
                        if (!string.IsNullOrEmpty(cust))
                        {
                                ...
                                if (listTemp.Count > 0)
                                {
                                    ...
                                    for (int i = 10; i <= worksheet.Dimension.End.Row; i++)
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(kd_brg_mp))
                                        {
                                            ...
                                            {
                                                ret.Errors.Add("Kode Barang MP (" + kd_brg_mp + ") tidak ditemukan");
                                            }
                                        }
                                        else
                                        {
                                            ret.Errors.Add("Kode barang marketplace tidak ditemukan lagi di baris " + i);
                                            ret.lastRow = i;
                                            break;
                                        }
                                    }
                                    if (ret.lastRow == 0)
                                        ret.lastRow = worksheet.Dimension.End.Row;
                                }
                                else
                                {
                                    var mp = MoDbContext.Marketplaces.Where(m => m.IdMarket.ToString() == customer.NAMA).FirstOrDefault();
                            ...
                            {
                                //customer not found
                                ret.Errors.Add("Akun marketplace tidak ditemukan");
                            }
                        }
                        else
                        {
                            //cust empty
                            ret.Errors.Add("Kode akun marketplace tidak ditemukan");
                        }
                        //}
                    }
                }
        ...
        public ActionResult HargaJualtoExcel()
        {
            ...
            try
            {
                using (var package = new OfficeOpenXml.ExcelPackage())
                {
                    ...
                    sSQL += "INNER JOIN MO..MARKETPLACE M ON A.NAMA = M.IDMARKET LEFT JOIN STF10 ON S.BRG = STF10.BRG ORDER BY NAMA,M.IDMARKET";
                    var dsBarang = EDB.GetDataSet("CString", "STF02", sSQL);
